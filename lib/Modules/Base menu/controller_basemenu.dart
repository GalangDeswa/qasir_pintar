...
class BasemenuController extends GetxController {
  ...
  fetchRiwayatPenjualan({id_toko}) async {
    print(
        '-------------------fetch riwayat penjualan base meu local---------------------');
  ...
  // fetchUserLocal(id) async {
  //   print('-------------------fetch user local BASE MENU---------------------');
  //
  //   List<Map<String, Object?>> query =
  //       await DBHelper().FETCH('SELECT * FROM users WHERE uuid = "$id"');
  //   // List<DataUser> user =
  ...
  //
  //   if (query.isNotEmpty) {
  //     datauser.value = query.map((e) => DataUser.fromJsondb(e)).toList();
  //     username.value = datauser.first.name!;
  //     email.value = datauser.first.email!;
  //     // uuid = datauser.first.uuid!;
  //     logo.value = datauser.first.logo!;
  //     toko.value = datauser.first.businessName!;
  //     return datauser;
  //   } else {
  //     print('empty');
  //     return null;
