...
import 'model_beban_laporan.dart';
import 'package:pdf/widgets.dart' as pw;

class BebanLaporanController extends GetxController {
  @override
  void onInit() {
  ...
  var summaryBeban = <BebanReportSummary>[].obs;
  ...
  ''';
    ...
    // debug
    print('Beban Detailed Report SQL: $sql');
    ...
    final List<BebanReportSummary> report = raw.map((row) {
      return BebanReportSummary.fromJsondb(row);
    }).toList();

    // 5) update your observable list
    summaryBeban.value = report;
  }
  ...
  Future<void> createPDF({
    required List<BebanReportSummary> summaryList,
    ...
    final Map<String, List<BebanReportSummary>> byDate = {};
              ...
              pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.Row(children: [
                      pw.Expanded(
                          child: pw.Text('nama beban: ${rpt.namaBeban}')),
                      pw.Expanded(
                          child: pw.Text('jumlah beban: ${rpt.countEntries}')),
                    ]),
                    pw.Divider(),
                    pw.Row(children: [
                      pw.Expanded(
                          child: pw.Text(
                              'jumlah: ${AppFormat().moneyFormat(rpt.totalAmount)}')),
                      pw.Expanded(
                          child: pw.Text('di input oleh: ${rpt.namaKaryawan}')),
                    ]),
                  ],
                ),
              ),
